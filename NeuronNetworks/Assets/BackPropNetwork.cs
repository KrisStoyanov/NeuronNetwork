...
public class BackPropNetwork
{
    ...
    public float[] expected = new float[1];
    ...
    public BackPropNetwork(int[] layers, string[] layerActivations)
    {
        this.activation = layerActivations;
        this.layers = new int[layers.Length];
        for (int i = 0; i < layers.Length; i++)
        {
            this.layers[i] = layers[i];
        }
        this.layerActivations = new string[layerActivations.Length];
        for (int i = 0; i < layerActivations.Length; i++)
        {
            this.layerActivations[i] = layerActivations[i];
        }
    ...
    public void BackPropagate()//backpropogation;
    {
        //float[] output = FeedForward(inputs);//runs feed forward to ensure neurons are populated correctly
        float[] output = FeedForward(inputValues);//runs feed forward to ensure neurons are populated correctly
    ...
    public int CompareTo(BackPropNetwork other) //Comparing For Genetic implementations. Used for sorting based on the fitness of the network
    {
    ...
    public BackPropNetwork Copy(BackPropNetwork nn) //For creating a deep copy, to ensure arrays are serialzed.
    {
        for (int i = 0; i < biases.Length; i++)
        {
            for (int j = 0; j < biases[i].Length; j++)
